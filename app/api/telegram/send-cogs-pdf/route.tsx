...
const formatRielPDF = (val: number) => `${Math.round(val || 0).toLocaleString('en-US')} ៛`;
const formatRielCaption = (val: number) => `${Math.round(val || 0).toLocaleString('en-US')} ៛`;

// --- 2. KHMER A4 STYLING (MATCHING WEB UI) ---
            ...
            <View key={seller} style={{ marginBottom: 15 }}>
              ...
              {rows.map((row: any, idx: number) => (
                <View key={idx} style={styles.tableRow} wrap={false}>
                  ...
                  <Text style={styles.colQty}>{Number(row.qty || 0).toLocaleString('en-US')}</Text>
                  <Text style={styles.colPrice}>{Number(row.cogs_price || 0).toLocaleString('en-US')}</Text>
                  <Text style={[styles.colTotal, row.isNegative ? { color: '#dc2626' } : {}]}>
                    {Math.round(row.calculatedAmount || 0).toLocaleString('en-US')}
                  </Text>
                </View>
              ))}
