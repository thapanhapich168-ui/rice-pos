...
export default function CogsReportPage() {
  ...
  const [bulkPaymentRows, setBulkPaymentRows] = useState<PaymentRow[]>([{ id: Date.now(), method: COGS_PAYMENT_WALLETS[0], amount: '' }]);
  ...
  async function processPayments(rows: PaymentRow[], targetDays: any[], isBulk: boolean) {
    ...
    for (const r of rows) {
       ...
       // 🔥 MATH FIX: Recognizes new wallet names as Liability deductions
       if (r.method.includes('Liability') || r.method.includes('Availability')) {
    ...
    try {
      ...
      if (botToken && masterChatId) {
        ...
        const owners = Array.from(new Set(targetDays.map(d => d.owner))).join(', ');
        
        let alertMsg = `🤝 *COGS SETTLEMENT PAID*\n`;
        alertMsg += `🏬 Branch ID: *${activeBranchId}*\n`;
        alertMsg += `📅 Date: ${new Date().toLocaleString('en-GB')}\n\n`;

        alertMsg += `👤 *Paid To:* ${owners}\n`;
        alertMsg += `📝 *Remarks:* ${isBulk ? 'Bulk via COGS Dashboard' : 'Inline via COGS Dashboard'}\n\n`;
        ...
        rows.forEach(r => {
           ...
           if (amt > 0) {
             ...
             alertMsg += `• Wallet: ${r.method}\n`;
             alertMsg += `• Amount: -${formatBal(amt)} ${symbol}\n\n`;
           }
        });
