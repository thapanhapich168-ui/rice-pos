          ...
          <form onSubmit={handleSubmit} style={styles.form}>
            ...
            <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0', marginTop: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <label style={{ fontSize: '12px', color: '#64748b', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Amount Spent</label>
              ...
              {paymentRows.map((row, index) => (
                <div key={row.id} style={{ display: 'flex', gap: '8px', marginBottom: '12px', alignItems: 'center' }}>
                  <select 
                    ...
                    style={{ width: '45%', padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '14px', outline: 'none', backgroundColor: '#fff', cursor: 'pointer', color: '#334155' }}
                  >
                  ...
                  <div style={{ flex: 1 }}>
                    <CurrencyInput 
                      ...
                      style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', boxSizing: 'border-box', outline: 'none', color: '#334155', fontSize: '14px', textAlign: 'right' }}
                    />
                  </div>
                  
                  {paymentRows.length > 1 && (
                    <button type="button" onClick={() => setPaymentRows(paymentRows.filter(r => r.id !== row.id))} style={{ background: 'none', border: 'none', color: '#ef4444', fontSize: '18px', cursor: 'pointer', padding: '0 4px' }}>✕</button>
                  )}
                </div>
              ))}
              ...
              {paymentRows.some(r => Number(r.amount) > 0) && (
                <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px dashed #cbd5e1', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  <span style={{ color: '#64748b', fontSize: '13px', fontWeight: 'bold' }}>Total Expense:</span>
                  <span style={{ color: '#ef4444', fontSize: '16px', fontWeight: 'bold' }}>{formatRiel(totalExpenseRiel)}</span>
                </div>
              )}
            </div>
        ...
        {activeTab === 'staff' && (
          <div>
            ...
            <form onSubmit={handleAddStaff} style={{ ...styles.form, padding: '16px', backgroundColor: '#f8fafc', borderRadius: '12px', marginBottom: '24px', border: '1px solid #e2e8f0' }}>
              <div style={{ fontWeight: 'normal', color: '#1b4d3e', marginBottom: '4px', fontSize: '14px' }}>➕ Register New Staff</div>
              <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <div style={{ flex: '1 1 200px' }}>
                  <label style={{ fontSize: '11px', fontWeight: 'normal', color: '#64748b', textTransform: 'uppercase', marginBottom: '6px', display: 'block' }}>Name</label>
                ...
                <div style={{ flex: '1 1 150px' }}>
                  <label style={{ fontSize: '11px', fontWeight: 'normal', color: '#64748b', textTransform: 'uppercase', marginBottom: '6px', display: 'block' }}>Monthly Salary (៛)</label>
            ...
            <div style={{ overflowX: 'auto', borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '1050px' }}>
                <thead style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #cbd5e1' }}>
                  <tr>
                    <th style={{ padding: '12px', color: '#64748b', fontSize: '12px', fontWeight: 'normal', textTransform: 'uppercase' }}>Name</th>
                    <th style={{ padding: '12px', color: '#64748b', fontSize: '12px', fontWeight: 'normal', textTransform: 'uppercase' }}>Start Date</th>
                    <th style={{ padding: '12px', color: '#64748b', fontSize: '12px', fontWeight: 'normal', textTransform: 'uppercase', textAlign: 'right' }}>Monthly Salary</th>
                    <th style={{ padding: '12px', color: '#10b981', fontSize: '12px', fontWeight: 'normal', textTransform: 'uppercase', textAlign: 'right' }}>Earned MTD</th>
                    <th style={{ padding: '12px', color: '#ef4444', fontSize: '12px', fontWeight: 'normal', textTransform: 'uppercase', textAlign: 'right' }}>Total Debt</th>
                    <th style={{ padding: '12px', color: '#3b82f6', fontSize: '12px', fontWeight: 'normal', textTransform: 'uppercase', textAlign: 'right' }}>Net Payout</th>
                    <th style={{ padding: '12px', color: '#b59410', fontSize: '12px', fontWeight: 'normal', textTransform: 'uppercase', textAlign: 'center', width: '280px' }}>➕ Add Advance/Debt</th>
                    <th style={{ padding: '12px', color: '#64748b', fontSize: '12px', fontWeight: 'normal', textAlign: 'center', width: '100px' }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {staffList.length === 0 ? (
                    <tr><td colSpan={8} style={{ textAlign: 'center', padding: '30px', color: '#94a3b8', fontSize: '14px' }}>No staff recorded yet.</td></tr>
                  ) : (
                    staffList.map((staff) => {
                      ...
                      return (
                        <tr key={staff.id} style={{ borderBottom: '1px solid #f1f5f9', backgroundColor: '#ffffff', transition: 'background 0.2s' }}>
                          ...
                          {/* 1. Name */}
                          <td 
                            style={{ padding: '12px', color: '#334155', cursor: 'text', fontSize: '14px' }}
                            onClick={() => { setEditingCell({ id: staff.id, field: 'name' }); setEditValue(staff.name); }}
                          >
                            {editingCell?.id === staff.id && editingCell?.field === 'name' ? (
                              <input autoFocus value={editValue} onChange={e => setEditValue(e.target.value)} onBlur={() => saveInlineEdit(staff.id, 'name')} onKeyDown={e => e.key === 'Enter' && saveInlineEdit(staff.id, 'name')} style={{...styles.tableInput, fontSize: '14px'}} />
                            ) : staff.name}
                          </td>

                          {/* 2. Start Date */}
                          <td 
                            style={{ padding: '12px', color: '#475569', cursor: 'text', fontSize: '14px' }}
                          ...
                          >
                            {editingCell?.id === staff.id && editingCell?.field === 'start_date' ? (
                              <input type="date" autoFocus value={editValue} onChange={e => setEditValue(e.target.value)} onBlur={() => saveInlineEdit(staff.id, 'start_date')} onKeyDown={e => e.key === 'Enter' && saveInlineEdit(staff.id, 'start_date')} style={{...styles.tableInput, fontSize: '14px'}} />
                            ) : (
                              <div>
                          ...
                          {/* 3. Monthly Salary */}
                          <td 
                            style={{ padding: '12px', color: '#475569', cursor: 'text', textAlign: 'right', fontSize: '14px' }}
                          ...
                          {/* 4. Total Earned */}
                          <td style={{ padding: '12px', color: '#10b981', textAlign: 'right', fontSize: '14px' }}>
                            {formatRiel(totalEarned)}
                          </td>

                          {/* 5. Total Debt */}
                          <td 
                            style={{ padding: '12px', color: '#ef4444', textAlign: 'right', cursor: 'text', fontSize: '14px' }}
                          ...
                          {/* 6. Net Payout */}
                          <td style={{ padding: '12px', color: isNegativePayout ? '#ef4444' : '#3b82f6', textAlign: 'right', fontSize: '14px' }}>
                          ...
                          <td style={{ padding: '12px', textAlign: 'center' }}>
                            <div style={{ display: 'flex', gap: '6px' }}>
                              <select 
                                ...
                                style={{ ...styles.tableInput, width: '85px', padding: '6px', fontSize: '13px' }}
                              >
                                <option value="Cash ៛">Cash ៛</option>
                                <option value="Cash $">Cash $</option>
                                <option value="QR ៛">QR ៛</option>
                              ...
                              <CurrencyInput 
                                ...
                                style={{...styles.tableInput, flex: 1, padding: '6px', fontSize: '13px'}} 
                              />
                              <button 
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 10000, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px', boxSizing: 'border-box' }} onMouseDown={() => setHistoryModal({ isOpen: false, staff: null, history: [] })}>
          <div style={{ backgroundColor: '#ffffff', width: '100%', maxWidth: '500px', borderRadius: '12px', padding: '24px', boxShadow: '0 10px 25px rgba(0,0,0,0.15)' }} onMouseDown={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f1f5f9', paddingBottom: '12px', marginBottom: '16px' }}>
              <h3 style={{ margin: 0, color: '#334155', fontSize: '18px', fontWeight: 'normal' }}>🕒 Debt History: {historyModal.staff.name}</h3>
              <button onClick={() => setHistoryModal({ isOpen: false, staff: null, history: [] })} style={{ background: 'none', border: 'none', fontSize: '18px', cursor: 'pointer', color: '#94a3b8' }}>✕</button>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid #cbd5e1' }}>
                      <th style={{ padding: '8px 0', textAlign: 'left', color: '#64748b', fontWeight: 'bold', fontSize: '12px' }}>Date</th>
                      <th style={{ padding: '8px 0', textAlign: 'left', color: '#64748b', fontWeight: 'bold', fontSize: '12px' }}>Payment Method</th>
                      <th style={{ padding: '8px 0', textAlign: 'right', color: '#64748b', fontWeight: 'bold', fontSize: '12px' }}>Amount (៛)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {historyModal.history.map((record) => (
                      <tr key={record.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={{ padding: '12px 0', textAlign: 'right', color: '#ef4444' }}>{formatRiel(record.amount)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '20px' }}>
              <button onClick={() => setHistoryModal({ isOpen: false, staff: null, history: [] })} style={{ padding: '10px 16px', backgroundColor: '#f1f5f9', border: 'none', borderRadius: '6px', cursor: 'pointer', color: '#475569', fontSize: '14px' }}>Close</button>
            </div>
          </div>
        </div>
      )}
