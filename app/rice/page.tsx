...
export default function RiceControl() {
  ...
  return (
    <div className="main-wrapper">
      ...
      {/* ORIGINAL IMPORT MODAL (Still accessible from Wholesale table actions) */}
      {importModal.isOpen && importModal.product && (
        <div className="modal-overlay" onMouseDown={() => setImportModal({ isOpen: false, product: null, add_qty: '', new_cost: '', new_price: '' })}>
          <div className="modal-content" style={{ maxWidth: '400px' }} onMouseDown={e => e.stopPropagation()}>
            <h2 style={{ marginTop: 0, marginBottom: '8px', color: '#1e293b' }}>📦 Quick Import Stock</h2>
            <p style={{ margin: '0 0 20px 0', fontSize: '14px', color: '#64748b' }}>Adding stock for: <b>{importModal.product.name}</b></p>
            <p style={{ fontSize: '12px', color: '#ef4444', marginBottom: '16px', fontWeight: 'bold' }}>Note: Quick imports are NOT tracked as business expenses automatically. Use the "Receive Stock" tab for full accounting.</p>
            ...
            <div style={{ marginTop: '24px', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              ...
              <button onClick={handleImportStock} disabled={!importModal.add_qty} style={{ padding: '10px 16px', background: importModal.add_qty ? '#3b82f6' : '#94a3b8', color: '#fff', border: 'none', borderRadius: '6px', cursor: importModal.add_qty ? 'pointer' : 'not-allowed', fontWeight: 'bold', fontSize: '14px' }}>Save Quick Import</button>
            </div>
          </div>
        </div>
      )}
